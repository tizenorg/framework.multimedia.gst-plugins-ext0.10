...
ecore_pipe_callback_handler (void *data, void *buffer, unsigned int nbyte)
{
	...
	if (!evaspixmapsink->visible) {
		GST_WARNING_OBJECT (evaspixmapsink,"visible is false..");
		return;
	}
...
static unsigned int
tbm_init_convert_bo_gemname(GstEvasPixmapSink *evaspixmapsink, unsigned int bo)
{
	int i = 0;
...
drm_fini_close_gem_handle(GstEvasPixmapSink *evaspixmapsink, Pixmap pixmap_id)
{
	...
	if (evaspixmapsink->drm_fd >= 0) {
		if (pixmap_id == 0) {
			...
			for (i = 0; i < MAX_GEM_BUFFER_NUM; i++) {
				if (evaspixmapsink->gem_info[i].dmabuf_fd > 0 && evaspixmapsink->gem_info[i].ref_pixmap == pixmap_id) {
					if (evaspixmapsink->buf_shared_type == BUF_SHARE_METHOD_FD) {
	...
	case GST_MAKE_FOURCC('S', 'R', '3', '2'):
	{
		...
		if (evaspixmapsink->evas_pixmap_buf->xvimage->data) {
			...
			if (scmn_imgb->buf_share_method == BUF_SHARE_METHOD_PADDR) {
				...
				/* export bo */
				if (img_data->bo[0]) {
					img_data->YBuf = tbm_init_convert_bo_gemname(evaspixmapsink, img_data->bo[0]);
				}
				if (img_data->bo[1]) {
					img_data->CbBuf = tbm_init_convert_bo_gemname(evaspixmapsink, img_data->bo[1]);
				}
				if (img_data->bo[2]) {
					img_data->CrBuf = tbm_init_convert_bo_gemname(evaspixmapsink, img_data->bo[2]);
				}
