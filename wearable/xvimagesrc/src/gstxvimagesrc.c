...
typedef struct
{
  void  *virtual_add[GEM_NAME_MAX];
  int gem_name[GEM_NAME_MAX];
  gint32 ion_fd[GEM_NAME_MAX];
  void *Ybo[GEM_NAME_MAX];
} MSM_GEM_MMAP;
#endif
typedef enum {
...
gst_xv_image_src_class_init (GstXVImageSrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SECURE_MODE,
     g_param_spec_boolean ("secure-mode",
     "secure mode session",
     "HDCP enabled",
     0, G_PARAM_READWRITE));
#endif
...
#else
static gint32 msm_convert_gem_to_fd(GstXVImageSrc *src, int *gemname_cnt, unsigned int gem_name, void *data)
{
  g_return_val_if_fail((data != NULL),0);
  int count=0;
  gint32 fd = 0;
  ...
  count = *gemname_cnt;
  GST_DEBUG("gamname_cnt = %d", count);
  GST_DEBUG ("tbm_bo_import %d", gem_name);
  MSM_GEM_MMAP *xv_gem_mmap = NULL;
  xv_gem_mmap = (MSM_GEM_MMAP *) data;
  if(count >=GEM_NAME_MAX)
    goto PASS;
  ...
  if (count < GEM_NAME_MAX ) {
    int i =0;
    for ( i =0 ; i < GEM_NAME_MAX ; i++) {
      if (gem_name == xv_gem_mmap->gem_name[i])
	goto PASS;
    }
    xv_gem_mmap->Ybo[count] = tbm_bo_import(src->bufmgr, gem_name);
    if (!xv_gem_mmap->Ybo[count]) {
      GST_ERROR ("[Error] : cannot import bo (key:%d)", gem_name);
      return 0;
    }
    bo_handle = tbm_bo_get_handle (xv_gem_mmap->Ybo[count], TBM_DEVICE_2D);
    if(!bo_handle.s32)
    {
      GST_ERROR ("[Error] : cannot get Ion FD");
      return 0;
    }
    xv_gem_mmap->ion_fd[count] = bo_handle.s32;
    src->framesize = tbm_bo_size(xv_gem_mmap->Ybo[count]);
#if 0 //Its no more required with Meta mode Encoder, Need to remove after meta mode verification
    tbm_bo_handle temp_virtual;
    temp_virtual = tbm_bo_get_handle (xv_gem_mmap->Ybo[count], TBM_DEVICE_CPU);
    xv_gem_mmap->virtual_add[count] = temp_virtual.ptr;

    if(!xv_gem_mmap->virtual_add[count])
    {
      GST_ERROR ("[Error] : cannot map virtual address");
      return 0;
    }
#endif
    /*set name*/
    xv_gem_mmap->gem_name[count] = gem_name;
  }
...
PASS:
  if (gem_name == xv_gem_mmap->gem_name[0]) {
    //*virtual_address = xv_gem_mmap->virtual_add[0];
    fd = xv_gem_mmap->ion_fd[0];
  } else if (gem_name == xv_gem_mmap->gem_name[1]) {
    //*virtual_address = xv_gem_mmap->virtual_add[1];
     fd = xv_gem_mmap->ion_fd[1];
  } else if (gem_name == xv_gem_mmap->gem_name[2]) {
    //*virtual_address = xv_gem_mmap->virtual_add[2];
     fd = xv_gem_mmap->ion_fd[2];
  }
  GST_DEBUG("ion_fd = %d", fd);
  return fd;
}
#endif
static void
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
#ifdef ENABLE_QC_SPECIFIC
    case PROP_SECURE_MODE:
      src->is_secured_mode = g_value_get_boolean (value);
      GST_DEBUG("is_secured_mode %d", src->is_secured_mode);
    break;
#endif
    default:
...
#ifdef ENABLE_QC_SPECIFIC
    case PROP_SECURE_MODE:
      g_value_set_boolean (value, src->is_secured_mode);
    break;
#endif
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
static void* gst_xv_image_src_update_thread (void * asrc)
{
  ...
  Atom atom_rotate=0;
  MSM_GEM_MMAP *xv_gem_mmap = NULL;
  xv_gem_mmap = (MSM_GEM_MMAP *)malloc(sizeof(MSM_GEM_MMAP));
  g_return_val_if_fail((xv_gem_mmap != NULL),NULL);

  memset(xv_gem_mmap, 0, sizeof(MSM_GEM_MMAP));
#else
...
#ifdef ENABLE_QC_SPECIFIC
    atom_rotate = XInternAtom (src->dpy, "_USER_WM_PORT_ATTRIBUTE_ROTATE_OFF", False);
#endif
    /*get data type*/
...
#ifdef ENABLE_QC_SPECIFIC
    XvSetPortAttribute (src->dpy, src->p, atom_rotate, 0);
#endif

#ifdef ENABLE_QC_SPECIFIC
    /* To be enabled for secure mode when msm is delivered */
    if(src->is_secured_mode) {
      atom_secure = XInternAtom (src->dpy, "_USER_WM_PORT_ATTRIBUTE_SECURE", False);
      XvSetPortAttribute (src->dpy, src->p, atom_secure, 1);
    }
#endif
...
next_event:
    ...
    if(src->format_id == FOURCC_RGB32) {
      ...
      else
      {
        ...
        {
#ifdef ENABLE_QC_SPECIFIC
          psimgb->fd[0] = msm_convert_gem_to_fd(src, &src->gemname_cnt, data->YBuf, xv_gem_mmap);
          GST_DEBUG("YBuf gem=%d to fd[0]=%d", data->YBuf, psimgb->fd[0]);
			  psimgb->buf_share_method = 1;//BUF_SHARE_METHOD_FD;
#else
...
#else
  for ( i=0 ; i < GEM_NAME_MAX ; i++) {
    if (xv_gem_mmap->Ybo[i]) {
      tbm_bo_unref(xv_gem_mmap->Ybo[i]);
      xv_gem_mmap->Ybo[i] = NULL;
    }
  }
  if (xv_gem_mmap) {
    free(xv_gem_mmap);
