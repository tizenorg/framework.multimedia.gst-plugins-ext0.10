...
static inline gboolean is_evas_image_object (Evas_Object *obj)
{
  const char *type;
  if (!obj) return FALSE;
...
GST_BOILERPLATE (GstEvasImageSink, gst_evas_image_sink, GstVideoSink,
    GST_TYPE_VIDEO_SINK);

static void gst_evas_image_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_evas_image_sink_get_property (GObject * object, guint prop_id,
...
gst_evas_image_sink_class_init (GstEvasImageSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_EVAS_OBJECT,
      g_param_spec_pointer ("evas-object", "Destination Evas Object",
          "Destination evas image object", G_PARAM_READWRITE));
...
gst_evas_image_sink_fini (gpointer data, GObject *obj)
{
  ...
  if (!esink) return;
  if (esink->epipe) ecore_pipe_del (esink->epipe);
  if (esink->oldbuf) gst_buffer_unref (esink->oldbuf);
}

static void
pipe_cb (void *data, void *buffer, unsigned int nbyte)
{
  GstBuffer *buf;
  ...
  if (nbyte != sizeof (GstBuffer *)) return;
  if (!esink->eo) return;
  ...
  if (esink->gl_zerocopy)
    {
       ...
       if (!img_data)
         {
            GST_ERROR ("Cannot get image data from evas object\n");
         }
       else
         {
...
static void
del_eo (void *data, Evas *e, Evas_Object *obj, void *event_info)
{
  GstEvasImageSink *esink = data;
...
static int
get_size_from_caps (GstCaps *caps, int *w, int *h)
{
  gboolean r;
  int width, height;
  GstStructure *s;

  if (!caps || !w || !h) return -1;
...
static void
gst_evas_image_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_EVAS_OBJECT:
      ...
      if (is_evas_image_object (eo))
        {
           esink->eo = eo;
           evas_object_event_callback_add (esink->eo, EVAS_CALLBACK_DEL, del_eo, esink);
           ...
           if (esink->gl_zerocopy)
             evas_object_image_content_hint_set (esink->eo,
                                                 EVAS_IMAGE_CONTENT_HINT_DYNAMIC);

	   if (!esink->epipe)
             esink->epipe = ecore_pipe_add (pipe_cb, esink);
	   if (!esink->epipe)
             {
		   GST_ERROR ("Cannot set evas-object property: "
				   "pipe create failed\n");
             }
        }
      else
        {
           GST_ERROR ("Cannot set evas-object property: "
                      "value is not evas image object\n");
        }
      break;
    default:
...
static void
gst_evas_image_sink_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstEvasImageSink *esink = GST_EVASIMAGESINK (object);
  ...
  if (esink->epipe)
    {
       ...
       if (r == EINA_FALSE) gst_buffer_unref (buf);
    }
  return GST_FLOW_OK;
}
...
{
  GST_DEBUG_CATEGORY_INIT (gst_evas_image_sink_debug, "evasimagesink",
      0, "Evas image object based videosink");

  return gst_element_register (evasimagesink, "evasimagesink", GST_RANK_NONE,
