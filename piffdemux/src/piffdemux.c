...
gst_piffdemux_class_init (GstPiffDemuxClass * klass)
{
  ...
  gst_piffdemux_signals[SIGNAL_LIVE_PARAM] =
      g_signal_new ("live-param", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
...
gst_piffdemux_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_PIFF_MEDIA_CAPS: {
	 ...
	 GST_DEBUG_OBJECT (piffdemux, "stream caps = %s", gst_caps_to_string(piffdemux->stream->caps));
    ...
    guint64 moof_offset, PiffDemuxStream * stream)
{
  ...
  while (traf_node) {
    ...
    while (uuid_node) {
      ...
      gst_byte_reader_init (&uuid_data, buffer, PIFF_UINT32 (buffer));
					...
					guint fps_n, guint fps_d, unsigned char *codec_data, unsigned int codec_data_len)
{
  ...
  GST_INFO_OBJECT (piffdemux, "prepared video caps : %s", gst_caps_to_string(caps));
}
					...
					guint sampling_rate, guint bps, guint channels, unsigned char *codec_data, unsigned int codec_data_len)
{
  ...
  piffdemux->stream->caps = caps;
  GST_INFO_OBJECT (piffdemux, "prepared audio caps : %s", gst_caps_to_string(caps));
